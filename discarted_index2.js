...
for (var i = 0; i < sourceFiles.length; i++) {
  startTime = process.hrtime();
  var sourceFilePath = path['octane'] + sourceFiles[i] + ".js";
  var destFilePath = path['transformed'] + sourceFiles[i] + ".min.js";
...
for (var i = 0; i < sourceFiles.length; i++) {
  ...
  fs.appendFileSync(path['transformed'] + "/validation_options.txt", message + "\n", "utf8")
  console.log(message);
}
...
for (var i = 0; i < sourceFiles.length; i++) {
  ...
  cp.execSync('cp ' + path['transformed'] + sourceFiles[i] + '.min.js ' + path['octane'] + "/" + sourceFiles[i] + '.js');
}
...
for (var j = 1; j <= jalangiCount; j++) {
  ...
  for (var i = 0; i < sourceFiles.length; i++) {
    ...
    var command = 'node ' + path['jalangi'] + 'src/js/commands/jalangi.js --inlineIID --inlineSource ' + analysis + ' ' + path['transformed'] + sourceFiles[i] + '.min.js >> ' + path['resultsTransformedJalangi'] + sourceFiles[i] + '.min.js_results.txt';
    // like running the command in terminal and output function defined above prints the result of the command
    exec(command, output);
    
    var hookCount = cp.execSync('grep -c J$. ' + path['transformed'] + sourceFiles[i] + '.min_jalangi_.js').toString();
    hookCount = hookCount.trimRight();
